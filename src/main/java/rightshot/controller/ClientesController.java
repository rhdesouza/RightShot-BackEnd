...
/*	@Secured({ Const.ROLE_ADMIN_ADMIN, Const.ROLE_CLIENTE })
	...
	@PostMapping("/getAllClientePageable")
	@Cacheable
	public ResponseEntity<Page<Cliente>> getAllClientePageable(Pageable pageable, @RequestBody(required=false) Cliente filter) {
		return Optional.ofNullable(clienteService.getClientePageable(filter, pageable))
				.map(result -> new ResponseEntity<Page<Cliente>>(result, HttpStatus.OK))
